...
def _register_via_api(
    ...
    """Test subject. kind=organization because person
    identities REQUIRE biometric proofing via
...
def test_person_register_is_bypass_blocked(
    ...
    """HARDENING CERTIFICATION: creating a PERSON via the
    legacy door is refused — persons must go through
    /identity/enroll (mandatory biometric proofing)."""
    status, payload = cluster.call(
        "POST",
        "/identity/register",
        {
